...
void EulerDatagram::deserializeData(Streamer &inputStreamer)
{
	...
	for (int i = 0; i < dataCount(); i++)
	{
		...
		euler = Eigen::AngleAxisf(rotation[0], Eigen::Vector3f::UnitX())
		...
		//quaternion.fromEulerAngles(euler);
  		Eigen::Quaternionf quaternion(euler);					
		...
		Eigen::Quaternionf tempQuat(quaternion.w(), quaternion.z(), quaternion.x(), quaternion.y());

		// covert from Euler to Quaternion again
		//euler.fromQuaternion(tempQuat);
		Eigen::Vector3f eu = (tempQuat.matrix()).eulerAngles(0,1,2);
...
*/
std::vector<eulerKinematics> EulerDatagram::getData()
{
  return m_data;
}