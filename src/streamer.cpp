...
Streamer::Streamer(const char* arr) 
	: m_offset(0)
{
...
void Streamer::read(int32_t &destination)
{
	...
	std::memcpy(byte_array, &m_array[m_offset], sizeof(destination));
...
void Streamer::read(int8_t &destination)
{
	...
	std::memcpy(&res, &m_array[m_offset], 1);
...
void Streamer::read(int16_t &destination)
{
	...
	// copy part of the original byte array into a temp array 
	std::memcpy(byte_array, &m_array[m_offset], 2);
...
void Streamer::read(std::string& str, int numChars)
{
	...
	std::memcpy(buffer, &m_array[m_offset], numChars);
	buffer[numChars] = '\0';
