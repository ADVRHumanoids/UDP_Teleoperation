...
void MetaDatagram::deserializeData(Streamer &inputStreamer)
{
	...
	for (int i=0; i < input.size(); i++)
	{
		...
		if (keyvalue.size() == 2) {
			...
			// apparently someone put some colons inthere
...
void MetaDatagram::printData() const
{
	...
	if (itemCount()) 
	{
		...
		if (hasItem("xmid"))
		{
			int did = std::stoi(itemData("xmid"),NULL,16);
...
std::string MetaDatagram::join(std::vector<std::string> strList, char const * delimiter) {
